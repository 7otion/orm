export class LocalStorageResultCache {
    ...
    get(key) {
        try {
            ...
            if (entry.expiresAt && Date.now() > entry.expiresAt) {
                if (this.debug)
                    console.log('[LocalStorageResultCache] EXPIRED (query)', key);
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error getting query:', error);
            return undefined;
        }
    }
        ...
        try {
            ...
            if (error instanceof DOMException &&
                ...
                try {
                    ...
                    if (this.debug)
                        console.warn('[LocalStorageResultCache] Retry failed:', retryError);
                }
            }
        }
    }
    getRowById(table, id) {
        try {
            ...
            if (!entry) {
                if (this.debug)
                    console.log(`[LocalStorageResultCache] MISS (row) ${table} id=${id}`);
                return undefined;
            }
            if (entry.expiresAt && Date.now() > entry.expiresAt) {
                if (this.debug)
                    console.log(`[LocalStorageResultCache] EXPIRED (row) ${table} id=${id}`);
                rows.delete(tableKey);
                this.saveRowsMap(rows);
                return undefined;
            }
            if (this.debug)
                console.log(`[LocalStorageResultCache] HIT (row) ${table} id=${id}`);
            return entry.row;
        }
        catch (error) {
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error getting row:', error);
            return undefined;
        }
    }
    setRowById(table, id, row) {
        try {
            ...
            this.saveRowsMap(rows);
            if (this.debug)
                console.log(`[LocalStorageResultCache] SET (row) ${table} id=${id}`, row);
        }
        catch (error) {
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error setting row:', error);
        }
    }
    invalidate(tables) {
        try {
            ...
            for (const table of tables) {
                ...
                if (this.debug)
                    console.log('[LocalStorageResultCache] INVALIDATE', table, `(${invalidated} entries)`);
            }
            this.saveQueriesMap(queries);
            this.saveTagMap(tagMap);
            this.saveRowsMap(rows);
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error invalidating:', error);
        }
    }
    clear() {
        try {
            ...
            if (this.debug)
                console.log('[LocalStorageResultCache] CLEAR ALL');
        }
        catch (error) {
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error clearing:', error);
        }
    }
    size() {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error getting size:', error);
            return 0;
        }
    }
    getQueriesMap() {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error parsing queries:', error);
            return new Map();
        }
    }
    saveQueriesMap(map) {
        try {
            localStorage.setItem(this.QUERY_CACHE_KEY, JSON.stringify([...map]));
        }
        catch (error) {
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error saving queries:', error);
        }
    }
    getTagMap() {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error parsing tags:', error);
            return new Map();
        }
    }
    saveTagMap(map) {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error saving tags:', error);
        }
    }
    getRowsMap() {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error parsing rows:', error);
            return new Map();
        }
    }
    saveRowsMap(map) {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error saving rows:', error);
        }
    }
    delete(key, tags) {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error deleting:', error);
        }
    }
    evictOldest() {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error evicting:', error);
        }
    }
    cleanup() {
        try {
            ...
            if (this.debug)
                console.warn('[LocalStorageResultCache] Error cleaning up:', error);
        }
    }
}
