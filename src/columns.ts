...
type IsRelationValue<V> = [NonNullable<V>] extends [ModelMarker]
	? true
	: [NonNullable<V>] extends [readonly ModelMarker[]]
		? true
		: false;
...
export type ColumnKeys<T> = Exclude<
	{
		[K in WritableKeys<T>]: T[K] extends (...args: any[]) => any
			? never
			: IsRelationValue<T[K]> extends true
				? never
				: K;
	}[WritableKeys<T>],
